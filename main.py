...
def tool_summarize(session_id: str, instruction: str = "") -> dict:
    s = sessions[session_id]
...
def tool_keyterms(session_id: str) -> dict:
    s = sessions[session_id]
...
def tool_quiz(session_id: str) -> dict:
    s = sessions[session_id]
...
def tool_export_summary(session_id: str, **_) -> dict:
...
async def agent_ask(req: AskRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        audit("tool_execute", req.session_id, {"call_id": call_id, "tool": tool_name})
        result = tool.fn(session_id=req.session_id, question=q)
        audit("tool_success", req.session_id, {"call_id": call_id, "tool": tool_name})
        return {"status": "success", "call_id": call_id, "tool_used": tool_name, "result": result}
    except Exception as e:
        audit("tool_error", req.session_id, {"call_id": call_id, "tool": tool_name, "error": str(e)})
